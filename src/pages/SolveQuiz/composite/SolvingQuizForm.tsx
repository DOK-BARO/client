...
export default function SolvingQuizForm({
  ...
  useEffect(() => {
    if (
      ...
      const haveCheckedOption = Object.values(checkedOptions).find(
        (value) => value,
      );
        ...
        {question.selectOptions.map((option, index) => {
          ...
          if (selectedRadioOption) {
            ...
            if (correctAnswer?.length) {
              // 채점되었고
              const correctAnswerIdx: string[] = correctAnswer.map((answer) =>
                (parseInt(answer) - 1).toString(),
              );
              isCorrect = correctAnswerIdx.includes(index.toString());

              if (isCorrect) {
                // 맞는 선지일때 (내가 선택한거랑 상관없이 초록색 적용)
      ...
      <div className={styles["title-area"]}>
        ...
        <div className={styles["title-wrapper"]}>
          {didAnswerChecked && (
            <div
              className={
                isCorrect ? styles["tag-correct"] : styles["tag-incorrect"]
              }
            >
              {isCorrect ? (
